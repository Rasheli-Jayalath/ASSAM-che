...
{
$phid=$_REQUEST['phid'];
$pdSQL1="SELECT phid, pid, al_file, ph_cap FROM t027project_plphotos  WHERE pid= ".$pid." and  phid = ".$phid;
...
@unlink($al_file.$al_file);
 mysql_query("Delete from t027project_plphotos where phid=".$_REQUEST['phid']);
 header("Location: sp_plphoto_input.php");
}
$size=50;
...
{ 
	...
	{
	$sql_pro=mysql_query("INSERT INTO t027project_plphotos(pid, al_file,ph_cap) Values(".$pid.", '".$file_name."', '".$ph_cap."' )");
	if ($sql_pro == TRUE) {
...
{
$ph_cap=$_REQUEST['ph_cap'];
$pdSQL = "SELECT a.phid, a.pid, a.al_file FROM t027project_plphotos a WHERE pid = ".$pid." and phid=".$phid." order by phid";
...
$old_al_file= $pdData["al_file"];
	...
	if(move_uploaded_file($_FILES['al_file']['tmp_name'],$target_file))
	{
    $sql_pro="UPDATE t027project_plphotos SET ph_cap='$ph_cap', al_file='$file_name' where phid=$phid";
	...
	{
	 $sql_pro="UPDATE t027project_plphotos SET ph_cap='$ph_cap' where phid=$phid";
...
<div class="box-set">
  ...
  <tr style="height:10%">
    ...
    <form action="sp_plphoto.php" method="post"><input type="submit" name="back" id="back" value="BACK" /></form></span></td></tr>
  <tr style="height:45%"><td align="center">
  <?php echo $message; ?>
  <form action="sp_plphoto_input.php" target="_self" method="post"  enctype="multipart/form-data">
  ...
  <table width="100%" class="table table-bordered">
                        ...
                        <tr>
						  ...
						  <?php if($adminflag==1)
								  {
								   ?>
						   <td align="right"><span style="float:left"><form action="sp_plphoto_input.php?phid=<?php echo $pdData['phid'] ?>" method="post"><input type="submit" name="edit" id="edit" value="Edit" /></form></span><span style="float:right"><form action="sp_plphoto_input.php?phid=<?php echo $pdData['phid'] ?>" method="post"><input type="submit" name="delete" id="delete" value="Del" onclick="return confirm('Are you sure?')" /></form></span>
						   <?php
						   }
						   ?></td>
                        </tr>
