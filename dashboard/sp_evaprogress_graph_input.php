...
if(isset($_REQUEST['save']))
{
	...
	$message="";
	$pgid=1;
	 $pdSQL = "SELECT ppg_id, pid, planned, actual, ppg_date,earned, cpi, spi,tcpi from t0211eva_progress_graph where pid = ".$pid." and ppg_date='$ppg_date'";
...
{
$sql_pro=mysql_query("INSERT INTO t0211eva_progress_graph (pid, planned, actual, ppg_date, earned, cpi, spi,tcpi) 
Values(".$pid.",".$planned.", ".$actual.", '".$ppg_date."'". ",". $earned .",".$cpi.",".$spi.",".$tcpi.")");
	if ($sql_pro == TRUE) {
...
$sql_pro="UPDATE t0211eva_progress_graph SET planned=$planned, actual=$actual, ppg_date='$ppg_date' , earned=$earned , cpi=$cpi , spi=$spi , tcpi=$tcpi where ppg_id=$ppg_id";
...
header("Location: sp_evaprogress_graph.php");
//}
}
...
$pdSQL1="SELECT ppg_id, pid, planned, actual, ppg_date ,earned, cpi, spi , tcpi from t0211eva_progress_graph  where pid = ".$pid." and  ppg_id = ".$ppg_id;
...
<div class="box-set">
  ...
  <tr style="height:10%"><td align="center" style="font-family:Verdana, Geneva, sans-serif; font-size:24px; font-weight:bold;"><span>Earned Value Analysis Graphical Data</span><span style="float:right"><form action="sp_evaprogress_graph.php" method="post"><input type="submit" name="back" id="back" value="BACK" /></form></span></td></tr>
  <tr style="height:45%"><td align="center">
  <?php echo $message; ?>
  <form action="sp_evaprogress_graph_input.php" target="_self" method="post" >
  ...
  <tr><td><label>Planned: (<?php echo $proj_cur;?>)</label></td><td><input  type="text" name="planned" id="planned" value="<?php echo $planned; ?>" /></td></tr>
  <tr><td><label>Actual: (<?php echo $proj_cur;?>)</label></td><td><input  type="text" name="actual" id="actual" value="<?php echo $actual; ?>" /></td></tr>
  <tr><td><label>Earned: (<?php echo $proj_cur;?>)</label></td><td><input  type="text" name="earned" id="earned" value="<?php echo $earned; ?>" /></td></tr>
  <tr><td><label>CPI:</label></td><td><input  type="text" name="cpi" id="cpi" value="<?php echo $cpi; ?>" /></td></tr>
  <tr><td><label>SPI:</label></td><td><input  type="text" name="spi" id="spi" value="<?php echo $spi; ?>" /></td></tr>
  <tr><td><label>TCPI-1:</label></td><td><input  type="text" name="tcpi" id="tcpi" value="<?php echo $tcpi; ?>" /></td></tr>
