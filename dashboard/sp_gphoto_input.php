...
@unlink($al_file.$al_file);
 mysql_query("Delete from t027project_gphotos where phid=".$_REQUEST['phid']);
...
{ 
	...
	{
	$sql_pro=mysql_query("INSERT INTO t027project_gphotos(pid, al_file,ph_cap) Values(".$pid.", '".$file_name."', '".$ph_cap."' )");
	if ($sql_pro == TRUE) {
...
<div class="box-set">
  ...
  <tr style="height:10%">
    ...
    <form action="sp_gphoto.php" method="post"><input type="submit" name="back" id="back" value="BACK" /></form></span></td></tr>
  <tr style="height:45%"><td align="center">
  <?php echo $message; ?>
  <form action="sp_gphoto_input.php" target="_self" method="post"  enctype="multipart/form-data">
  ...
  <table width="100%" class="table table-bordered">
                        ...
                        <tr>
						  ...
						  <?php if($adminflag==1)
								  {
								   ?>
						   <td align="right"><span style="float:left"><form action="sp_gphoto_input.php?phid=<?php echo $pdData['phid'] ?>" method="post"><input type="submit" name="edit" id="edit" value="Edit" /></form></span><span style="float:right"><form action="sp_gphoto_input.php?phid=<?php echo $pdData['phid'] ?>" method="post"><input type="submit" name="delete" id="delete" value="Del" onclick="return confirm('Are you sure?')" /></form></span>
						   <?php
						   }
						   ?></td>
                        </tr>
