...
{
mysql_query("Delete from t0171interchanges where brid=".$_REQUEST['brid']);
}


 $pdSQL = "SELECT brid, pgid, pid, serial, description, total, inprogress, completed FROM t0171interchanges where pid = ".$pid." order by serial";
...
<div class="box-set">
  ...
  <tr style="height:10%"><td align="center" style="font-family:Verdana, Geneva, sans-serif; font-size:24px; font-weight:bold;"><span>Interchanges/ Services Areas</span><span style="float:right"><form action="chart1.php" method="post"><input type="submit" name="back" id="back" value="BACK" /></form></span></td></tr>
 
  <tr style="height:100%"><td align="center">
  <?php if($adminflag==1)
								  {
								   ?>
  <span style="text-align:right; "><form action="sp_interchanges_input.php" method="post"><input type="submit" name="add_new " id="add_new" value="Add New Record" /></form></span>
  <?php
  }
  ?>
  ...
  <table width="100%" class="table table-bordered">
                        ...
                        <tr>
						  ...
						  <?php if($adminflag==1)
								  {
								   ?>
						   <td align="right"><span style="float:left"><form action="sp_interchanges_input.php?brid=<?php echo $pdData['brid'] ?>" method="post"><input type="submit" name="edit" id="edit" value="Edit" /></form></span><span style="float:right"><form action="sp_interchanges.php?brid=<?php echo $pdData['brid'] ?>" method="post"><input type="submit" name="delete" id="delete" value="Del" onclick="return confirm('Are you sure?')" /></form></span></td>
						   <?php
						   }
						   ?>
                        </tr>
