...
if(isset($_REQUEST['save']))
{
	...
	$message="";
	$pgid=1;
$sql_pro=mysql_query("INSERT INTO mis_tbl_2_components (pid, code, detail) Values(".$pid.",'".$serial."', '".$description."' )");
	if ($sql_pro == TRUE) {
...
$sql_pro="UPDATE mis_tbl_2_components SET code='$serial', detail='$description' where cid=$cid";
...
header("Location: sp_component.php");
}
if(isset($_REQUEST['cid']))
{
$cid=$_REQUEST['cid'];

$pdSQL1="SELECT cid, code, detail FROM mis_tbl_2_components  where pid = ".$pid." and  cid = ".$cid;

$pdSQLResult1 = mysql_query($pdSQL1) or die(mysql_error());
$pdData1 = mysql_fetch_array($pdSQLResult1);

	$serial = $pdData1['code'];
	$description = $pdData1['detail'];
...
<div class="box-set">
  ...
  <tr style="height:10%">
    ...
    <form action="sp_component.php" method="post"><input type="submit" name="back" id="back" value="BACK" /></form></span></td></tr>
  <tr style="height:45%"><td align="center">
  <?php echo $message; ?>
  <form action="sp_component_input.php" target="_self" method="post" >
 ...
 <tr><td colspan="2"> <?php if(isset($_REQUEST['cid']))
	 {
