...
<div class="box-set">
  ...
  <tr style="height:10%">
    <td align="center" style="font-family:Verdana, Geneva, sans-serif; font-size:24px; font-weight:bold;"><span>Videos</span><span style="float:right"><form action="chart1.php" method="post"><input type="submit" name="back" id="back" value="BACK" /></form></span></td></tr>
  <tr style="height:45%"><td align="center">
    <?php if($adminflag==1)
								  {
								   ?>
  <span style="text-align:right; vertical-align:top; margin:0; padding:0 "><form action="sp_video_input.php" method="post"><input type="submit" name="add_new " id="add_new" value="Manage Videos" /></form></span>
  <?php
  }
  ?>
  ...
  <table width="650" class="table table-bordered">
			...
			if(mysql_num_rows($pdSQLResult) >= 1){
				...
				{*/
				?>
				 <a href="sp_video_large.php?video=<?php echo $result['al_file'];?>&phid=<?php echo $result['phid'];?>" target="_parent" >
                 <img src="video_file_icon.jpg" width="150" height="100" border="0" /></a>
				<?php
				//}
				//else
				//{
				?>
                <?php /*?> <a href="sp_video_large.php?photo=<?php echo $result['al_file'];?>&phid=<?php echo $result['phid'];?>" target="_parent" >
                 <img src="images/tag_small.png"  border="0" width="150" height="100"/></a><?php */?>
                 <?php
				 //}
				}?></td>
            <?php 
