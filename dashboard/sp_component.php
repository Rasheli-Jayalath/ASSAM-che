...
{
mysql_query("Delete from mis_tbl_2_components where cid=".$_REQUEST['cid']);
}
...
$pname = $pdData['proj_name'];
 ...
 $pdSQL = "SELECT cid, code, detail FROM mis_tbl_2_components where pid = ".$pid;
$pdSQLResult = mysql_query($pdSQL);
...
<div class="box-set">
  ...
  <tr style="height:10%">
    <td align="center" style="font-family:Verdana, Geneva, sans-serif; font-size:24px; font-weight:bold;"><span>Components</span><span style="float:right">
  ...
  <tr style="height:100%"><td align="center">
   <?php if($adminflag==1)
	{
	 ?>
  <span style="text-align:right; "><form action="sp_component_input.php" method="post"><input type="submit" name="add_new " id="add_new" value="Add New Record" /></form></span>
  <?php
  }
  ?>
 <div style="overflow-y: scroll; height:400px;">
  <table width="100%" class="table table-bordered">
							  ...
							  { ?>
                        <tr>
                          <td align="center"><?php echo $pdData['code'];?></td>
                          <td align="left"><?php echo $pdData['detail'];?></td>
                          <?php if($adminflag==1)
								  {
								   ?>
						   <td align="right"><span style="float:left"><form action="sp_component_input.php?cid=<?php echo $pdData['cid'] ?>" method="post"><input type="submit" name="edit" id="edit" value="Edit" /></form></span><span style="float:right"><form action="sp_component.php?cid=<?php echo $pdData['cid'] ?>" method="post"><input type="submit" name="delete" id="delete" value="Del" onclick="return confirm('Are you sure?')" /></form></span></td>
						   <?php
						   }?>
                        </tr>
						<?php
