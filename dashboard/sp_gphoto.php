...
<div class="box-set">
  ...
  <tr style="height:45%"><td align="center">
    <?php if($adminflag==1)
								  {
								   ?>
  <span style="text-align:right; vertical-align:top; margin:0; padding:0 "><form action="sp_gphoto_input.php" method="post"><input type="submit" name="add_new " id="add_new" value="Manage Photos" /></form></span>
  <?php
  }
  ?>
  ...
  <table width="650" class="table table-bordered">
			...
			if(mysql_num_rows($pdSQLResult) >= 1){
				...
				{
				?>
				 <a href="sp_gphoto_large.php?photo=<?php echo $result['al_file'];?>&phid=<?php echo $result['phid'];?>" target="_parent" ><img src="<?php echo $data_url.$result['al_file']; ?>" width="150" height="100" border="0" /></a>
				<?php
				}
				else
				{
				?>
                 <a href="sp_gphoto_large.php?photo=<?php echo $result['al_file'];?>&phid=<?php echo $result['phid'];?>" target="_parent" >
