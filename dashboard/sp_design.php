...
//===============================================
 ...
 $pdSQL = "SELECT dgid, pgid, pid, serial, description, total, submitted, revision, approved, approvedpct FROM t0101designprogress where pid = ".$pid." order by serial";
...
<div class="box-set">
  ...
  <tr style="height:10%"><td align="center" style="font-family:Verdana, Geneva, sans-serif; font-size:24px; font-weight:bold;"><span>Design Progress</span><span style="float:right"><form action="chart1.php" method="post"><input type="submit" name="back" id="back" value="BACK" /></form></span></td></tr>
 
  <tr style="height:100%"><td align="center">
   <?php if($adminflag==1)
								  {
								   ?>
  <span style="text-align:right; "><form action="sp_design_input.php" method="post"><input type="submit" name="add_new " id="add_new" value="Add New Record" /></form></span>
  <?php
  }
  ?>
  ...
  <table width="100%" class="table table-bordered">
                              <thead>
                                <tr>
                                  ...
                                  <th width="5%" style="text-align:center">Submitted By Contractor</th>
								  <th width="5%" style="text-align:center">Under Revision</th>
								  <th width="5%" style="text-align:center">Approved by NHA</th>
                        ...
                        <tr>
                          ...
                          <td align="right"><?php echo number_format($pdData['approvedpct'],2)."%";?></td>
						    <?php if($adminflag==1)
								  {
								   ?>
						   <td align="right"><span style="float:left"><form action="sp_design_input.php?dgid=<?php echo $pdData['dgid'] ?>" method="post"><input type="submit" name="edit" id="edit" value="Edit" /></form></span><span style="float:right"><form action="sp_design.php?dgid=<?php echo $pdData['dgid'] ?>" method="post"><input type="submit" name="delete" id="delete" value="Del" onclick="return confirm('Are you sure?')" /></form></span></td>
						   <?php
						   }
						   ?>
                        </tr>
