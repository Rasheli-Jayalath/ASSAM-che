...
<div class="box-set">
  ...
  <tr style="height:10%"><td align="center" style="font-family:Verdana, Geneva, sans-serif; font-size:24px; font-weight:bold;"><span>Project Layout Plan</span><span style="float:right"><form action="chart1.php" method="post"><input type="submit" name="back" id="back" value="BACK" /></form></span></td></tr>
  <tr style="height:100%"><td align="center">
   <?php if($adminflag==1 && $sql_num==0)
								  {
								   ?>
  <span style="text-align:right; "><form action="sp_align_input.php" method="post"><input type="submit" name="add_new " id="add_new" value="Add New Record" /></form></span>
  <?php
  }
  else if($adminflag==1 && $sql_num>=1)
  {?>
   <span style="text-align:right; "><form action="sp_align_input.php?alid=<?php echo $pdData["alid"]; ?>" method="post"><input type="submit" name="edit_new " id="edit_new" value="Update " /></form></span>
  <?php }
  ?>
