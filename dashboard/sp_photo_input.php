...
{
$phid=$_REQUEST['phid'];
$pdSQL1="SELECT phid, pid, al_file, ph_cap FROM t027project_photos  WHERE pid= ".$pid." and  phid = ".$phid;
...
@unlink($al_file.$al_file);
 mysql_query("Delete from t027project_photos where phid=".$_REQUEST['phid']);
...
{ 
	...
	{
	$sql_pro=mysql_query("INSERT INTO t027project_photos(pid, al_file,ph_cap) Values(".$pid.", '".$file_name."', '".$ph_cap."' )");
	if ($sql_pro == TRUE) {
...
{
$ph_cap=$_REQUEST['ph_cap'];
$pdSQL = "SELECT a.phid, a.pid, a.al_file FROM t027project_photos a WHERE pid = ".$pid." and phid=".$phid." order by phid";
...
$old_al_file= $pdData["al_file"];
	...
	if(move_uploaded_file($_FILES['al_file']['tmp_name'],$target_file))
	{
    $sql_pro="UPDATE t027project_photos SET ph_cap='$ph_cap', al_file='$file_name' where phid=$phid";
	...
	{
	 $sql_pro="UPDATE t027project_photos SET ph_cap='$ph_cap' where phid=$phid";
...
<div class="box-set">
  ...
  <table width="100%" class="table table-bordered">
                              ...
                              <tbody>
							  <?php
						 $pdSQL = "SELECT phid, pid, al_file, ph_cap FROM t027project_photos WHERE pid = ".$pid." order by phid";
						 $pdSQLResult = mysql_query($pdSQL);
						$i=0;
