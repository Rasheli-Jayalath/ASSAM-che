...
{
mysql_query("Delete from t0071pc1summary where pc1_id=".$_REQUEST['pc1_id']);
}
...
<div class="box-set">
  ...
  <tr style="height:10%"><td align="center" style="font-family:Verdana, Geneva, sans-serif; font-size:24px; font-weight:bold;"><span>PC1 Summary</span><span style="float:right"><form action="chart1.php" method="post"><input type="submit" name="back" id="back" value="BACK" /></form></span></td></tr>
 
  <tr style="height:100%"><td align="center">
   <?php if($adminflag==1)
	{
	 ?>
  <span style="text-align:right; "><form action="sp_pc1_input.php" method="post"><input type="submit" name="add_new " id="add_new" value="Add New Record" /></form></span>
  <?php
  }
  ?>
 <div style="overflow-y: scroll; height:400px;">
  <table width="100%" class="table table-bordered">
							  ...
							  { ?>
                        <tr>
                          <td align="center"><?php echo $pdData['item_id'];?></td>
                          <td align="left"><?php echo $pdData['description'];?></td>
                          <td align="right"><?php if($pdData['f1']!=""&&$pdData['f1']!=NULL)echo number_format($pdData['f1'],2);?></td>
                          <td align="right"><?php if($pdData['f2']!=""&&$pdData['f2']!=NULL)
						   echo number_format($pdData['f2'],2);?></td>
                          <td align="right"><?php if($pdData['f3']!=""&&$pdData['f3']!=NULL)
						  echo number_format($pdData['f3'],2);?></td>
                          <td align="right"><?php if($pdData['price']!=""&&$pdData['price']!=NULL)
						  echo number_format($pdData['price'],2);?></td>
						    <?php if($adminflag==1)
								  {
								   ?>
						   <td align="right"><span style="float:left"><form action="sp_pc1_input.php?pc1_id=<?php echo $pdData['pc1_id'] ?>" method="post"><input type="submit" name="edit" id="edit" value="Edit" /></form></span><span style="float:right"><form action="sp_pc1.php?pc1_id=<?php echo $pdData['pc1_id'] ?>" method="post"><input type="submit" name="delete" id="delete" value="Del" onclick="return confirm('Are you sure?')" /></form></span></td>
						   <?php
						   }?>
                        </tr>
						<?php
