...
if(isset($_REQUEST['save']))
{
	...
	$message="";
	$pgid=1;
$sql_pro=mysql_query("INSERT INTO t0201finstatus (pid,serial,description, amount) Values(".$pid.",'".$serial."', '".$description."',".$amount.")");
	if ($sql_pro == TRUE) {
...
$sql_pro="UPDATE t0201finstatus SET serial='$serial', description='$description', amount=$amount where finid=$finid";
...
header("Location: sp_finstatus.php");
}
if(isset($_REQUEST['finid']))
{
$finid=$_REQUEST['finid'];

$pdSQL1="SELECT finid, pid, serial, description, amount FROM t0201finstatus  where pid = ".$pid." and  finid = ".$finid;
...
<div class="box-set">
  ...
  <tr style="height:10%"><td align="center" style="font-family:Verdana, Geneva, sans-serif; font-size:24px; font-weight:bold;"><span>Financial Status</span><span style="float:right"><form action="sp_finstatus.php" method="post"><input type="submit" name="back" id="back" value="BACK" /></form></span></td></tr>
  <tr style="height:45%"><td align="center">
  <?php echo $message; ?>
  <form action="sp_finstatus_input.php" target="_self" method="post" >
  ...
  <tr><td><label>Item ID:</label></td><td><input  type="text" name="serial" id="serial" value="<?php echo $serial; ?>" /></td></tr>
    ...
    <tr><td><label>Description:</label></td><td><input  type="text" name="description" id="description" value="<?php echo $description; ?>" /></td></tr>
     ...
     <tr><td><label>Expenditure: (<?php echo $proj_cur;?>)</label></td><td><input  type="text" name="amount" id="amount" value="<?php echo $amount; ?>" /></td></tr>
	
 <tr><td colspan="2"> <?php if(isset($_REQUEST['finid']))
	 {
		 
	 ?>
     <input type="hidden" name="finid" id="finid" value="<?php echo $_REQUEST['finid']; ?>" />
