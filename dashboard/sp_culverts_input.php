...
if(isset($_REQUEST['save']))
{
	...
	$message="";
	$pgid=1;
$sql_pro=mysql_query("INSERT INTO t0171culverts (pid, serial, description, total, inprogress, completed) Values(".$pid.",".$serial.", '".$description."',".$total.",".$inprogress.", ".$completed.")");
	if ($sql_pro == TRUE) {
...
$sql_pro="UPDATE t0171culverts SET serial='$serial', description='$description', total=$total,  inprogress=$inprogress,  completed=$completed where brid=$brid";
...
header("Location: sp_culverts.php");
}
if(isset($_REQUEST['brid']))
{
$brid=$_REQUEST['brid'];

$pdSQL1="SELECT brid, pgid, pid, serial, description, total, inprogress, completed FROM t0171culverts  where pid = ".$pid." and  brid = ".$brid;
...
<div class="box-set">
  ...
  <tr style="height:10%"><td align="center" style="font-family:Verdana, Geneva, sans-serif; font-size:24px; font-weight:bold;"><span>Culverts / Underpasses</span><span style="float:right"><form action="sp_culverts.php" method="post"><input type="submit" name="back" id="back" value="BACK" /></form></span></td></tr>
  <tr style="height:45%"><td align="center">
  <?php echo $message; ?>
  <form action="sp_culverts_input.php" target="_self" method="post" >
