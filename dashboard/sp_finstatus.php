...
{
mysql_query("Delete from t0201finstatus where finid=".$_REQUEST['finid']);
}
...
$pname = $pdData['proj_name'];
 ...
 $pdSQL = "SELECT finid, pgid, pid, serial, description, amount FROM t0201finstatus where pid = ".$pid;
$pdSQLResult = mysql_query($pdSQL);
...
<div class="box-set">
  ...
  <tr style="height:100%"><td align="center">
   <?php if($adminflag==1)
	{
	 ?>
  <span style="text-align:right; "><form action="sp_finstatus_input.php" method="post"><input type="submit" name="add_new " id="add_new" value="Add New Record" /></form></span>
  <?php
  }
  ?>
  ...
  <table width="100%" class="table table-bordered">
                              <thead>
                                <tr>
                                  ...
                                  <th width="10%" style="text-align:center">Expenditure (<?php echo $proj_cur;?>)</th>
								   <?php if($adminflag==1)
								  {
								   ?>
                        ...
                        <tr>
                          ...
                          <td align="right"><?php echo number_format($pdData['amount'],0);?></td>
						    <?php if($adminflag==1)
								  {
								   ?>
						   <td align="right"><span style="float:left"><form action="sp_finstatus_input.php?finid=<?php echo $pdData['finid'] ?>" method="post"><input type="submit" name="edit" id="edit" value="Edit" /></form></span><span style="float:right"><form action="sp_finstatus.php?finid=<?php echo $pdData['finid'] ?>" method="post"><input type="submit" name="delete" id="delete" value="Del" onclick="return confirm('Are you sure?')" /></form></span></td>
						   <?php
						   }?>
                        </tr>
						<?php
