...
{ 
	...
	$check_res_g=mysql_query($check_query_g);
	$pdData_g = mysql_fetch_array($check_res_g);
	...
	{
	$sql_pro=mysql_query("INSERT INTO  project_photos(pid, al_file,ph_cap,lcid,date_p,giscode) 
...
function getCanals(lid)
{
	...
	if (lid!=0) {
			var strURL="findcanal_p.php?lid="+lid;
			var req = getXMLHTTP();
...
<table align="center">
  ...
  <tr><td><label><?php echo LOCATION.$lcid;?>:</label></td>
  <td><select id="ph_cap" name="ph_cap" onchange="getCanals(this.value)">
  ...
  <?php if($_SESSION['ne_user_type']==1)
							{
								?>
    <a class="SubmitButton"  href="javascript:void(null);" onclick="window.open('location_form.php', '<?php echo PLOAD_PHOTO_BTN;?>','width=670px,height=550px,toolbar=0,menubar=0,location=0,status=0,scrollbars=0,resizable=0,left=0,top=0');"  style="margin:5px; text-decoration:none"><?php echo ADD_LOC;?></a>
  <?php
							}
							?>
