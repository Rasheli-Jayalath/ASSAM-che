...
if(isset($_REQUEST['delete']))
{
	...
	$sql="select attachment from t013nonconformitynotice where nos_id=".$_REQUEST['nos_id'];
...
mysql_query("Delete from t013nonconformitynotice where nos_id=".$_REQUEST['nos_id']);
 $activity="Non Confirmity Record -".$_REQUEST['nos_id']."-  Deleted Successfully";
...
<?php echo COMP; ?>:  <select id="comp_id" name="comp_id" onchange="getDates(this.value)" style="width:242px">
  ...
  <input type="button"  onclick="doFilter(this.form);" class="SubmitButton" name="Submit" id="Submit" value=" <?php echo VIEW; ?> " />
   ...
   <?php if($pid != ""&&$pid!=0){?> <a href="project_nonconfirmity_input.php"  class="button"><?php echo ADD_NEW_REC;?></a><?php } 
			    ...
			    if($arightr[0]==$user_cd)
						{
                           ...
                           <?php
								 ...
								 if(isset($_REQUEST['iss_status']) && $_REQUEST['iss_status']!="")
							 {
								  ...
								  $pdSQL = "SELECT nos_id, pid, iss_no, iss_title, iss_detail, iss_status, iss_action, iss_remarks, attachment, comp_id FROM t013nonconformitynotice where pid=$pid and comp_id=$cmpid and iss_status=$status";
							 }
							 else
							 { 
							 $pdSQL = "SELECT nos_id, pid, iss_no, iss_title, iss_detail, iss_status, iss_action, iss_remarks, attachment, comp_id FROM t013nonconformitynotice where pid=$pid and comp_id=$cmpid";
							 }
							   $pdSQL .= " order by iss_no desc";
