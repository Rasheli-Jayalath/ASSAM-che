...
elseif($div_id==2) $level="Activity ";
  ...
  echo $tempquery = "select itemid from boqdata where parentgroup='$parentgroup' ";
				...
				{
				 $str_g_query = "select * from boqdata WHERE activitylevel=".$div_id." AND parentcd=$parentcd";
				$str_g_result = mysql_query($str_g_query);
				  $levlcount = mysql_num_rows($str_g_result);
				 if($levlcount>0)
				{   $tempdata2 = mysql_fetch_array($str_g_result);
					$parentcd2=$tempdata2["itemid"];
				}
				if($parentcd2!=0&&$parentcd2!="")
				{
				 $str_g_query2 = "select * from boqdata WHERE  parentcd=$parentcd2";
